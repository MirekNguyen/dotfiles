...
function dark-mode
  if test -z (cat $HOME/.config/dotfiles/config/kitty/kitty.conf | grep 'Gruvbox Material Light Soft')
    osascript -e 'tell app "System Events" to tell appearance preferences to set dark mode to false'
    kitty +kitten themes --reload-in=all 'Gruvbox Material Light Soft'
    perl -i -p -e 's/dark/light/g' "$HOME"/.config/nvim/lua/options.lua
    ...
    perl -i -p -e 's/light/dark/g' "$HOME"/.config/nvim/lua/options.lua
