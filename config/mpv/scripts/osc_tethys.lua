...
local user_opts = {
   ...
   scalewindowed = 0.7, -- scaling of the controller when windowed
...
local tethys = {
   -- Config
   skipBy = 10, -- skipback/skipfrwd amount in seconds
   ...
   seekbarFgColor = "FFFFFF", -- #d73d48 -- 483DD7
...
-- "{\\1c&H"..color.."}"
local tethysStyle = {
   button = ("{\\blur0\\bord0\\1c&H%s\\3c&HFFFFFF\\fs(%d)\\fn(%s)}"):format(tethys.buttonColor, tethys.buttonH,
      tethys.osdSymbolFont),
   buttonHovered = genColorStyle(tethys.buttonHoveredColor),
   buttonHoveredRect = ("{\\rDefault\\blur0\\bord0\\1c&H%s\\1a&H%X&}"):format(tethys.buttonHoveredRectColor,
      tethys.buttonHoveredRectAlpha),
   smallButton = ("{\\blur0\\bord0\\1c&H%s\\3c&HFFFFFF\\fs(%d)\\fn(%s)}"):format(tethys.buttonColor,
      tethys.smallButtonSize, tethys.osdSymbolFont),
   trackButton = ("{\\blur0\\bord0\\1c&H%s\\3c&HFFFFFF\\fs(%d)\\fn(%s)}"):format(tethys.buttonColor,
      tethys.trackButtonSize, tethys.osdSymbolFont),
   windowBar = ("{\\1c&H%s}"):format(tethys.windowBarColor),
   windowButton = ("{\\blur0\\bord(%d)\\1c&H%s\\3c&H000000\\fs(%d)\\fn(%s)}"):format(tethys.windowTitleOutline,
      tethys.windowButtonColor, tethys.windowButtonSize, tethys.osdSymbolFont),
   closeButtonHovered = genColorStyle(tethys.closeButtonHoveredColor),
   windowTitle = ("{\\blur0\\bord(%d)\\1c&H%s\\3c&H000000\\fs(%d)}"):format(tethys.windowTitleOutline, tethys.textColor,
      tethys.windowTitleSize),
   buttonTooltip = ("{\\blur0\\bord(1)\\1c&H%s\\3c&H000000\\fs(%d)}"):format(tethys.tooltipColor,
      tethys.buttonTooltipSize),
   timecode = ("{\\blur0\\bord0\\1c&H%s\\3c&HFFFFFF\\fs(%d)}"):format(tethys.textColor, tethys.timecodeSize),
   cacheText = ("{\\blur0\\bord0\\1c&H%s\\3c&HFFFFFF\\fs(%d)}"):format(tethys.textColor, tethys.cacheTextSize,
      tethys.osdSymbolFont),
   seekbar = ("{\\blur0\\bord0\\1c&H%s\\3c&HFFFFFF\\fs(%d)}"):format(tethys.seekbarFgColor, tethys.seekbarHeight),
   seekbarTimestamp = ("{\\blur0\\bord(%d)\\1c&H%s\\3c&H000000\\fs(%d)}"):format(user_opts.tooltipborder,
      tethys.textColor, tethys.seekbarTimestampSize),
...
layouts["box"] = function()
   ...
   lo.geometry = { x = posX + pos_offsetX - (25 * 2) - osc_geo.p,
...
layouts["slimbox"] = function()
   ...
   lo = add_layout("tc_left")
   lo.geometry = { x = posX - (inner_w / 2) + osc_geo.r, y = posY + 1,
      an = 7, w = tc_w, h = ele_h }
...
function osc_init()
   ...
   ne.eventresponder["mbtn_right_up"] = function() show_message(mp.get_property_osd("filename")) end
   ...
   ne.eventresponder["mbtn_left_down"] = function() mp.commandv("seek", -tethys.skipBy, tethys.skipMode) end
   ne.eventresponder["shift+mbtn_left_down"] = function() mp.commandv("frame-back-step") end
   ne.eventresponder["mbtn_right_down"] = function() mp.commandv("seek", -tethys.skipByMore, tethys.skipMode) end
         ...
         if (state.tc_ms) then
            return (
                mp.get_property_osd("playback-time/full") .. " / " .. "-" ..
                    mp.get_property_osd("playtime-remaining/full"))
         else
